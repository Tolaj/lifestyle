// components/InputFields/index.js
...
const Button = (props) => {
    return (<>
        <button onClick={props.onClick} type={props.type ? props.type : "submit"} className={` w-full text-white  focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm px-5 py-2.5 text-center  ${props.styleBg ? props.styleBg : "bg-blue-600 hover:bg-blue-700 focus:ring-blue-300"}`}>{props.label}</button>
    </>)
}
            ...
            return (<>
                <div className="text-center ">
                    <button type="submit" data-tour="group-btn" onClick={() => { props.clickButton(props.route) }} className={` ${props.className} md:px-6 md:py-4 px-3 py-2 md:gap-3 gap-1 shadow-lg md:shadow-none bg-white flex items-center  text-black active:bg-slate-200 text-sm font-medium  rounded-full  hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-fit ease-linear transition-all duration-150 `}>
                        ...
                        {props?._as?.user?.groups?.find((group) => group._id === (typeof window !== 'undefined' ? localStorage.getItem('projectLifestyle_activeGroup') : ''))?.members.length || 0}
                    </button>
                </div>
            </>)
        default:
        ...
        case "ADD_CART":
            return (
                <div
                    ...
                    className={`${props.className} inline-grid place-items-center`}
                >
                    {/* Cart Icon */}
                    <svg
                        ...
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        className="w-6 h-6 text-black hover:cursor-pointer col-start-1 row-start-1"
                    >
                        <path d="M2.25 2.25a.75.75 0 0 0 0 1.5h1.386c.17 0 .318.114.362.278l2.558 9.592a3.752 3.752 0 0 0-2.806 3.63c0 .414.336.75.75.75h15.75a.75.75 0 0 0 0-1.5H5.378A2.25 2.25 0 0 1 7.5 15h11.218a.75.75 0 0 0 .674-.421 60.358 60.358 0 0 0 2.96-7.228.75.75 0 0 0-.525-.965A60.864 60.864 0 0 0 5.68 4.509l-.232-.867A1.875 1.875 0 0 0 3.636 2.25H2.25ZM3.75 20.25a1.5 1.5 0 1 1 3 0 1.5 1.5 0 0 1-3 0ZM16.5 20.25a1.5 1.5 0 1 1 3 0 1.5 1.5 0 0 1-3 0Z" />
                    </svg>
                    ...
                    <svg
                        ...
                        viewBox="0 0 16 16"
                        fill="currentColor"
                        className="size-3 bg-white rounded-full col-start-1 row-start-1 self-start justify-self-end "
                    >
                        <path
                            fillRule="evenodd"

...
function CartListModal(props) {
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await FetchAPI(process.env.SERVER_API+"/api/orders", 'POST', additionalData);
      setCheckoutData({})
      props._as.setCart([])
      props._as.setToast("Order Placed successfully!")
              ...
              <div class="flex h-full flex-col overflow-y-scroll bg-white shadow-xl ">
                ...
                <div class="border-t border-gray-200 px-4 py-6 sm:px-6">
                  ...
                  <div class="mt-6">
                      ...
                      disabled={Object.values(tempData).length>0?0:1} class={` ${Object.values(tempData).length>0? '' : 'opacity-10'}  w-full flex items-center justify-center rounded-md border border-transparent bg-[#161616] px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-[#2b2a2a]`}>Place Order</button>
                  </div>

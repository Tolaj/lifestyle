            ...
            <div className="w-full  mx-auto items-center flex justify-between md:flex-nowrap flex-wrap md:px-10 px-4">            
               ...
               {windowWidth<700?<>
                    ...
                    <div className='absolute bottom-8 right-8'>
                            ...
                            <div className='flex flex-col items-end'>                               
                                {/* <div className={`${mobileButtonToggle?"":"hidden"}`}>
                                    {props._acTab.tabButtons.length>0? props._acTab.tabButtons[props._acTab.activeTabSection].map((button,index)=>{
                                        return(<>                        
                                            <InputFields.TabButton type={props._acTab.tabButtonType[props._acTab.activeTabSection][index]} clickButton={props._acTab.setModalToggle} placeholder={button} />
                                        </>)
                                    }):<></>} 
                                </div> */}
                ...
                <ul className={`mt-6 md:mt-0   ${props._acTab.tabSections.length>1?"border-b":""}  flex flex-wrap  text-sm font-medium text-center text-gray-500  border-gray-200 dark:border-gray-700 dark:text-gray-400`}>
                    ...
                    {props._acTab.tabButtons.length>0? props._acTab.tabButtons[props._acTab.activeTabSection].map((button,index)=>{
                        return(<>                        
                            <InputFields.TabButton type={props._acTab.tabButtonType[props._acTab.activeTabSection][index]} clickButton={props._acTab.setModalToggle} placeholder={windowWidth<700?"Add":button} />
                        </>)
                    }):<></>}
                </ul>   
                </>}           

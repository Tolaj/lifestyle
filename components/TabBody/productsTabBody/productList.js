// components/TabBody/productsTabBody/productList.js
  ...
  if (windowWidth < 700) {
    ...
    const columnComponentSplit = (params) => {
      ...
      const [tempData, setTempData] = React.useState(() => {
        ...
        const allMemberIds = activeGroupMembers?.map((member) => member._id) || [];
        const splitAmong = params.data.splitAmong?.length ? params.data.splitAmong : allMemberIds;
        if (!params.data.splitAmong?.length) params.node.setDataValue('splitAmong', splitAmong);
        return { ...params.data, splitAmong };
      });

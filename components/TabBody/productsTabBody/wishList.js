// components/TabBody/productsTabBody/wishList.js
...
function WishList(props) {
  ...
  const dataFilter = (params) => {
    ...
    return params.data.filter((data) => myGroup.wishlists.includes(data._id));
  }
  ...
  const ActionsComponent = (params) => {
    ...
    const handleAction = async (method, data) => {
      try {
        const response = await FetchAPI('/api/wishlists', method, data);
    ...
    return (<>
      <div className="flex items-start pt-2 justify-center md:h-full gap-1 md:w-full md:gap-4">
        <InputFields.GridButton title="Add to cart" type="ADD_CART"
          onClick={() => {
            let tempWishlistData = JSON.parse(JSON.stringify(params?.data))
            delete tempWishlistData?._id
            delete tempWishlistData?.createdAt
            delete tempWishlistData?.updatedAt
            delete tempWishlistData?.__v
            params.data.createdBy = tempWishlistData?.createdBy?._id
            params.data.paidBy = tempWishlistData?.paidBy?._id

            FetchAPI(process.env.SERVER_API + "/api/categories", 'GET')
              .then(data => {
                let productList = []
                if (data) {
                  tempWishlistData?.items.map((item) => {
                    delete item?._id
                    item.splitAmong = item.splitAmong.map(user => user._id);
                    item.product.category = data.find(category => category._id === item.product.category);
                    item.product.splitAmong = item.splitAmong
                ...
                const cartData = JSON.parse(localStorage.getItem("projectLifestyle_cart_" + localStorage.getItem('projectLifestyle_activeGroup')) || '[]');
                if (cartData.length > 0) {
                  alert(" You can't mix two wishlist items. Cart needs to be empty!")
                } else {
                  ...
                  props._as.setCart(productList)
                  localStorage.setItem("projectLifestyle_cart_checkout", JSON.stringify(tempWishlistData))
                }
              })
              .catch(error => {
        ...
        <InputFields.GridButton title="Edit" type="EDIT" onClick={() => { params._as.setModalToggle('/admin/wishList'); params?._as?.setWishList(params.data) }} className="w-6 h-6 text-black hover:cursor-pointer" />
        ...
        <InputFields.GridButton title="Read More" type="ARROW" onClick={() => { params.setReadMore(params.readMore == params.index + 'i' ? null : params.index + 'i') }} className={`w-5 h-5 md:hidden  text-black ${params.readMore == params.index + 'i' ? "rotate-90" : ""}`} />
      </div>
    </>);
  };
  ...
  if (windowWidth < 700) {
    ...
    const columnComponentItems = (params) => {
      ...
      return (<>
        <div onClick={() => { setShowItems(showItems === params.data._id ? null : params.data._id) }} className='w-full h-6 border-2 rounded-xl my-1 hover:bg-gray-100 flex items-center justify-center'>
        ...
        <div className={` ${showItems === params.data._id ? "" : "hidden"}  divide-y  `}>
    ...
    return (<>
      <GridBody columns={['name', 'items', 'totalPrice', 'date', 'paidBy', 'createdBy', 'action']} columnComponents={[columnComponentName, columnComponentItems, '', '', columnComponentPaidBy, columnComponentCreatedBy]} ActionsComponent={ActionsComponent} setFormData={props._as.setCategoryData} colProps={['', { autoHeight: true }]} _as={props._as} dataFilter={dataFilter} gridApi={process.env.SERVER_API + "/api/wishlists"} />
    </>);
  }

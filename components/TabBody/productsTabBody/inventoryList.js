// components/TabBody/productsTabBody/inventoryList.js
...
export default function InventoryList(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await FetchAPI(process.env.SERVER_API + "/api/categories", 'GET');
        setCategory(data);
        setCategorySm(data);
      } catch (error) {
  ...
  if (windowWidth < 700) {
    ...
    const columnComponentIcon = (params) => {
      ...
      const data = params.paramPass.categoryData;
